          ...
          <v-container>
            <v-row>
              ...
              <v-col cols="12" class="text-center">
                <span class="subtitle-1  font-weight-black">最小值←→最大值</span>
                <v-row>
                  <v-col cols="12" sm="2"
                    ><v-text-field
                      v-model="minmax[0]"
               ...
               <!-- critical min max 設定 -->
              <v-col cols="12" class="text-center">
                <span class="subtitle-1" style="color:red;">危險下限值←→危險上限值</span>
                <v-row>
                  <v-col cols="12" sm="2"
                    ><v-text-field
                      v-model="minmax_critical[0]"
                      type="number"
                      @change="
                        () => {
               ...
               <!-- warning min max 設定 -->
              <v-col cols="12" class="text-center">
                <span class="subtitle-1" style="color:orange;">警戒下限值←→警戒上限值</span>
                <v-row>
                  <v-col cols="12" sm="2"
                    ><v-text-field
                      v-model="minmax_warning[0]"
                      type="number"
                      @change="
                        () => {
                          limit_warning.key += 1;
...
export default {
    ...
    minmax_critical:{
      get: function () {
        if (this.addDialog==true) {
          ...
          return [this.editedItem.critical_min,Number(this.editedItem.critical_max)];
        }
      },
      set: function (newValue) {
      if (this.addDialog==true) {
  ...
  methods: {
    ...
    editShow: function(data) {
      ...
      data.warning_max= data.warning_max == null ? data.critical_max : data.warning_max;
    ...
    editsubmit: async function() {
      ...
      console.log(parm);
      await this.$axios
        .patch(`https://61.56.172.10/col-data/${this.editedItem.id}/`, parm, {
          httpsAgent: agent
        })
        .then(res => {
          if (res.data == "修改成功") {
    ...
    inputchk_addmin(val){//add min 最小值 check
      ...
      this.$nextTick(() => {
        ...
        //其餘等於原值
        this.addItem.min =
          objitem < 0 ? 0 : objitem >= this.addItem.max ? Number(this.addItem.max) : objitem;
      });
    },
    inputchk_addmax(val){//add max 最大值 check
