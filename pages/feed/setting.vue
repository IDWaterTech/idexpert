        ...
        <v-row class="mx-3 my-3 mt-3">
          ...
          <v-col cols="12" sm="8" class="my-2">
            <v-autocomplete
              ...
              clearable
              @change="manselect"
              ><v-icon slot="prepend" @click="getmanudata"
                >mdi-reload</v-icon
              ></v-autocomplete
            >
            <v-card>
        ...
        <v-row class="mx-3 my-3 mt-3">
          <v-col cols="12" sm="6" class="my-2">
            <v-autocomplete
              ...
              "
              @change="ficselect"
              ><v-icon slot="prepend" @click="getficdata"
                >mdi-reload</v-icon
              ></v-autocomplete
            >
            成份清單
            ...
            >
              <v-chip
                class="mx-3"
                v-for="n in fing.filter(
                  x => x.feed_ingredient_category_id == fic_idx
                )"
                :key="n.id"
                :value="n.id"
              >
            ...
            <v-card class="mt-1">
              <v-toolbar flat color="lightblue" dark>
                ...
                <v-toolbar-title class="font-weight-light">
                  成份類別{{
                    feed_ingredient_category.filter(x => x.id == fic_idx)
                      .length > 0
                      ? "：" +
                        feed_ingredient_category.filter(x => x.id == fic_idx)[0]
                          .name_ch
                      : ficisEditing
                      ? "：新增模式"
                      : ""
          ...
          <v-col cols="12" sm="6" class="my-2">
            <v-card :disabled="fic_idx == null || fic_idx == undefined">
              <v-toolbar flat color="lightblue" dark>
                ...
                <v-toolbar-title class="font-weight-light">
                  成份<span v-if="fingisEditing"
                    >-{{ fingmode == "add" ? "新增模式" : "編輯" }}</span
                  >
                </v-toolbar-title>
                <v-spacer></v-spacer>
                <v-btn
              ...
              <v-card-text>
                ...
                >
                  <v-text-field
                    v-show="false"
                    v-model="fingfield.feed_ingredient_category_id"
                    dense
                ...
                <v-dialog v-model="dialog.param" width="400px">
                  <v-card>
                    ...
                    <v-form ref="parmform" v-model="parmvalid">
                      ...
                      <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn dark color="primary" @click="parmsubmit"
                          >新增</v-btn
                        >
                      </v-card-actions>
                    </v-form>
        ...
        <v-row class="mx-3 my-3 mt-3">
          ...
          <v-col cols="12" sm="10" class="my-2">
            ...
            <v-autocomplete
              ...
              clearable
              @change="comboselect"
              ><v-icon slot="prepend" @click="getcombodata"
                >mdi-reload</v-icon
              ></v-autocomplete
            >
            <v-card>
              <v-toolbar flat color="lightblue" dark>
                ...
                <v-btn
                  ...
                  @click="
                    () => {
                      main_formula = {};
                      (sub_formula = {}), (comboisEditing = !comboisEditing);
                    }
                  "
              ...
              <v-card-text>
                ...
                >
                      ...
                      <v-alert outlined dense color="purple">
                        <div class="body-1">
                          ...
                          <span
                            v-if="
                              ficwithdetail_main
                                .filter(x => x.id == item)[0]
                                .hasOwnProperty('parameters') == false
                            "
                  ...
                  <v-row
                    dense
                    v-for="item in combofield.sub_items"
                    :key="item"
                    justify="center"
                    align="center"
...
export default {
  ...
  methods: {
    ...
    ficselect: function() {
      if (
        this.feed_ingredient_category.filter(x => x.id == this.fic_idx).length >
        0
      ) {
        //edit mode
        this.ficmode = "edit";
