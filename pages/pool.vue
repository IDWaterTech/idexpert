              ...
              <div class="card-title ma-2 pa-0 cursor-pointer">
                <v-row style="margin-bottom: 0;">
                  <v-col class="pa-1" cols="12" md="8" sm="6">
                    ...
                    <!-- 財務 -->
                    <!-- <div class="header-bar" style="padding: 0;border-bottom: none">
...
export default {
  ...
  methods: {
              ...
              if(step.actionList&&step.actionList.length>0) {
                ...
                step.actionList.forEach(action=>{
                  ...
                  if(action.dailyCheckList&&action.dailyCheckList.length>0) {
                    ...
                    action.execute=3;
                    action.start_date = action.dailyCheckList[0].scheduling_date;
    ...
    async getDisease() {
      ...
      console.log('疾病檢驗清單',_.cloneDeep(this.diseaseReport));
      this.getWater();
    },
    // 取得水質檢驗報告
    async getWater() {
    ...
    reportEditOpen(item) {//開啟檢驗報告編輯
      ...
      setTimeout(async ()=>{
        ...
        if(item.type == 1) {//1是疾病檢驗2是水質檢驗
          ...
          // console.log("bacteriaAll:",this.bacteriaAll);
          // console.log("addReport 1:",_.cloneDeep(this.addReport[0]));
          // this.addReport[0].species = this.addReport[0].species_id;//資料來源是species_id
          // this.addReport[0].species = undefined,
          // this.bacteriaAll.forEach(x=>{//所有物種：如蝦、魚
          //   x.test.forEach(t=>{//的檢驗方法IIPC、PCR…的ID
          //     if(t.id==this.addReport[0].method_id) {//與編輯資料上的方法ID相同
          //       let ids = [];
          //       t.disease.forEach(d=>ids.push(d.id));//該方法可檢驗的所有疾病ID
          //       let isIncludes = [];
    ...
    select(item,id,bool=false,type) {
      // console.log('select',item,id,bool,type);
        ...
        if(bool) {
            ...
            if(type=='method') {
                // console.log('method',this.bacteriaAll);
                // debugger;
    ...
    async submitreport() {
      ...
      console.log('submitreport 0:',parm);
      this.isSelectPool = true;
      if(parm.pond_ids) {
